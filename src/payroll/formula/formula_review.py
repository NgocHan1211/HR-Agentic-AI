...
def review_formula(store: FormulaCandidateStore, candidate_id: str, decision: ReviewStatus, reviewer: str, validation_context: ValidationContext, *, note: str = "", evidence_ref: str | None = None) -> FormulaCandidate:
    ...
    candidate.review_history.append({"from_status": candidate.review_status.value, "decision": decision.value, "reviewer": reviewer, "note": note, "evidence_ref": evidence_ref, "at": datetime.now(timezone.utc).isoformat()})
