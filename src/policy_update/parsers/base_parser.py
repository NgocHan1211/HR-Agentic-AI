...
class BaseParser(ABC):
    ...
    def parse(self, request: ParseRequest) -> ParsedDocument:
        ...
        try:
            ...
            raise
        # Bắt thêm io.UnsupportedOperation và ValueError khi stream không hỗ trợ seek()
        except (AttributeError, OSError, ValueError, io.UnsupportedOperation) as exc:
    ...
    def validate_request(self, request: ParseRequest) -> None:
        ...
        if request.extension.lower() not in {ext.lower() for ext in self.supported_extensions}:
            raise UnsupportedFileType(
                message=f"{self.parser_name} does not support extension {request.extension!r}",
        ...
        if request.declared_mime_type is not None:
            ...
            if request_mime not in supported_mimes:
                raise InvalidMimeType(
                    message=f"Invalid MIME type: {request.declared_mime_type}",
                    source_id=request.source_ref.source_id,
                    details={
                        "declared_mime_type": request.declared_mime_type,
        ...
        if request.document_role not in self.supported_roles:
            raise UnsupportedFileType(
                message=f"{self.parser_name} does not support document role {request.document_role.value!r}",
                source_id=request.source_ref.source_id,
